...
[资料](https://www.cnblogs.com/AhuntSun-blog/p/12745183.html)参考[AhuntSun](https://github.com/)
...
​	git add ./	将内容放到暂存区，（其实先将git对象放置到版本库）

​	git commit -m	将暂存区提交的版本库

​		git diff	查看哪些地方发生了修改，还没有被暂存

​		git diff --staged	查看哪些东西缓存了还没提交

​	git commit -a -m 	-a可以直接跳过缓存区，将文件提交到版本库

​	git log	查看历史记录

​	git rm	删除工作区的文件，并将文件提交到缓存区

​	git mv	重命名，并提交到暂存区

**git分支**

​	分支的本质其实就是一个提交对象，head是一个指针，默认指向master分支

​	创建分支

​		<u>git branch xxx</u>	创建一个分支

​		<u>git checkout xxx</u>	切换分支,切换分支会改变工作目录

​				**每次切换分支前，当前分支一定是干净的状态，就是已经提交的状态**

​		git branch -D xxx	强制删除分支

​		git branch name commitchash		在指定的对象上创建指定分支

​		git checkout -b xxx		新建一个分支并切换过去

**分支的合并**

​		切换到主分支，合并分支

​		git merge xxx		快进合并，不会产生冲突

​		git merge  xxx		产生冲突，处理冲突，提交

**git存储**

​		git stash 		会将未完成的任务保存修改
...
👉 工作流有各式各样的用法，但也正因此使得在实际工作中如何上手使用变得很头大。这篇指南通过总览公司团队中最常用的几种`Git`工作流。
...
如果开发团队成员已经很熟悉`Subversion`，集中式工作流让你无需去适应一个全新流程就可以体验`Git`带来的收益。这个工作流也可以作为向更`Git`风格工作流迁移的友好过渡。
